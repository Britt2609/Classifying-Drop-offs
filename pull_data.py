...
def load_data(data_source, input_filepath, pos_or_neg , short):
    ...
    for file in files:
        ...
        if data_source == 'madesmart' and pos_or_neg == 'positive':
            if short:
                df_temp = pd.read_csv(file, usecols=['Date and UTC Time', 'Latitude', 'Longitude' , 'SOG', 'COG'], nrows =1000)
            else:
            ...
            if short:
                df_temp = pd.read_csv(file, usecols = df_columns, nrows = 10000)
            else:
...
def main(input_filepath, output_filepath):
    ...
    df_negative = df_negative.sample(n = df_positive.shape[0]*100, random_state=101)
